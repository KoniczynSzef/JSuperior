...
import { fetchQuiz } from '@/utils/fetchFunctions';
...
export async function generateMetadata({
    ...
    const lesson: Lesson | null = await getLesson(parseInt(params.id));
    return {
        title: lesson?.title,
    };
}
...
const page: FC<pageProps> = async ({ params = { id: '1' } }) => {
    ...
    const lesson: Lesson | null = await getLesson(parseInt(params.id));
    const prevLesson: Lesson | null = await getLesson(parseInt(params.id) - 1);
    const nextLesson: Lesson | null = await getLesson(parseInt(params.id) + 1);
